describe('Adding Products', () => {
  it('should navigate to the about page', () => {
    ...
    cy.get('a[href*="caramel-apple"]').click()
    ...
    // Open cart - verify details in Cart.jsx
    cy.get('span.cart-item-qty').click()
    cy.get('span.cart-num-items').contains('2 items')

    // Verify mini cart shows relevant details
    cy.get('span.cart-num-items').contains('(2 items)')
    cy.get('span.item-desc > span').contains('Caramel Apple')
    cy.get('span.totals').contains('$19.50')
    cy.get('div.total > h3').contains('$39.00')
  })
})
